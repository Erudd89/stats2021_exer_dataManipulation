...
-   List each variable and the level of measurement. Note if there are
    ...
    -   research, nominal, no issue
    -   height, ratio, no issue  
    -   weight, ratio, no issue
    -   lvst1, ordinal, no issue  
    -   lvst2, ordinal, no issue
    -   lvst3, ordinal, no issue  
    -   lvst4, ordinal, no issue
    -   frst1, ordinal, no issue  
    -   frst2, ordinal, no issue  
    -   frst3, ordinal, no issue
...
![](BR_Week03DataManipulationAssignment_files/figure-markdown_github/unnamed-chunk-10-1.png)

``` r
hist(dataset$FOSS_total)
```

![](BR_Week03DataManipulationAssignment_files/figure-markdown_github/unnamed-chunk-10-2.png)

``` r
barplot(dataset$LOSS_total)
```

![](BR_Week03DataManipulationAssignment_files/figure-markdown_github/unnamed-chunk-10-3.png)

``` r
barplot(dataset$FOSS_total)
```

![](BR_Week03DataManipulationAssignment_files/figure-markdown_github/unnamed-chunk-10-4.png)
...
-   How do the z-scores compare between the sum scores and the mean
    ...
    -   Answer: The mean is able to adjust for missing values. The sum
        is requires the same number of answers for each variable.
